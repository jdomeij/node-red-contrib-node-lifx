...
LightItem.prototype.updateColorTemp = function updateColorTemp(input, output) {
  ...
  if (typeof input.ct === 'number') {
    kelvin = 1000000 / limitValue(input.ct, 125, 500);
    changed = true;
  }

  else if (typeof input.mirek === 'number') {
    kelvin = 1000000 / limitValue(input.mirek, 125, 500);
    changed = true;
  }

  else if (typeof input.mired === 'number') {
    kelvin = 1000000 / limitValue(input.mired, 125, 500);
    changed = true;
  }

  // Kelvin color temperature
  else if (typeof input.kelvin === 'number') {
    kelvin = input.kelvin;
    changed = true;
  }
  ...
  // Brightness
  if (typeof input.bri === 'number') {
    bri = limitValue(input.bri, 0, 100);
  }
  else if (typeof input.brightness === 'number') {
    bri = limitValue(input.brightness, 0, 100);
  }
  ...
  kelvin = limitValue(kelvin, 2000, 8000);
